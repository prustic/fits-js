...
The Node path uses `NodeFileReader` from the `@fits-js/core/node` subpath.
The subpath exists so browser bundlers do not have to walk past
`node:fs/promises` when tree-shaking. Everything else lives at
`@fits-js/core`.
