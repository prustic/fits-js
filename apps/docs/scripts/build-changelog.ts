// Generates apps/docs/src/content/docs/changelog.mdx from
// packages/*/CHANGELOG.md. Runs with Node 22+ native TypeScript stripping;
// no bundler, no regex.
//
// Data sources:
...
const REPO_URL = "https://github.com/prustic/fits-js";
...
const PACKAGES: readonly Package[] = [{ name: "@fits-js/core", dir: "packages/fits-core" }];
...
function pushBullet(entry: VersionEntry, text: string): void {
  ...
  // Skip workspace dependency-bump bullets like "@fits-js/core@0.1.0".
  if (cleaned && !cleaned.startsWith("@fits-js/")) {
...
// Each release has a `@fits-js/core@VERSION` git tag created by changesets on
// publish. Use that as the canonical release date.
