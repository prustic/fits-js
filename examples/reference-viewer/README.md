...
grayscale. Display contrast comes from a linear stretch between the 0.5%
and 99.5% percentile of the finite pixel values, the standard baseline
for "show me what's there." NaN renders black. The image is Y-flipped
...
a viewer, but it's not how lazy access is meant to be used in
production. See [`http-range`](../http-range/) for the lazy-cutout
pattern.
